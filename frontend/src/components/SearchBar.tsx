...
export const SearchBar: React.FC<SearchBarProps> = ({ onSearch, initialQuery = '', searchHistory = [] }) => {
  ...
  const filteredHistory = searchHistory.filter(s => 
  ...
  const finalSuggestions = [...new Set([...filteredHistory, ...filteredApi])].slice(0, 7);
      ...
      {showDropdown && (
        <ul className="absolute top-full mt-2 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg z-10 overflow-hidden">
          ...
          {finalSuggestions.map(suggestion => (
            ...
            >
              <Icon name={searchHistory.includes(suggestion) ? 'history' : 'search'} className="w-4 h-4 text-gray-400 flex-shrink-0" />
